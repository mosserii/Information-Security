...
void handle_connection(int conn)
{
    ...
    if (!read_data(conn, (byte*)&size, sizeof(size))) //read first 4 bytes and then reverse them 
        ERROR("failed to receive size");

    size = ntohl(size);

    if (!read_data(conn, (byte*)buff, size)) //read size bytes which is the message, including \0 byte
        ERROR("failed to receive data");

    fprintf(LOG_FP, "%s\n", buff); //writes into LOG_FP the content of buff
    fflush (LOG_FP);
